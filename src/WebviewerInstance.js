...
const App = () => {
  ...
  useEffect(() => {
    WebViewer(
      ...
      const { documentViewer, Annotations, annotationManager, Tools, Math  } = instance.Core;
      ...
      class TriangleCreateTool extends Tools.GenericAnnotationCreateTool {
        ...
        deserialize(element, pageMatrix) {
          ...
          this.vertices = storedVertices.map(v => new Math.Point(v.x, v.y));
        }
  ...
  const setActiveTool = (tool) => {
    console.log({tool, webViewerInstance});    
    webViewerInstance.setToolMode(tool);
  }

  const setFitMode = (fitBy) => {
    console.log({fitBy, webViewerInstance});    
